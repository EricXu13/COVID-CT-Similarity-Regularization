...
from models import build_model, BYOL_Constraint_Wrapper
from lr_scheduler import build_scheduler
from optimizer import build_optimizer
from logger import create_logger
from utils import load_checkpoint, load_checkpoint_no_resume, save_checkpoint, get_grad_norm, auto_resume_helper, reduce_tensor, ScalingHelper
...
def parse_args():
    ...
    parser.add_argument('--general-cfg', type=str, default='configs/cfg_constraint_level.yaml', help='path to general config file')
    ...
    parser.add_argument('--projection-size', type=int, help="Projection Size for SR calculation")
    parser.add_argument('--decay-min-rate', type=float, help="Contrastive regularization factor decay")
    parser.add_argument('--max-rate', type=float, help="Initial factor for similarity reg")

    parser.add_argument('--resume', help='resume from checkpoint')
    parser.add_argument('--accumulation-steps', type=int, help="gradient accumulation steps")
    parser.add_argument('--amp-opt-level', type=str, default='O1', choices=['O0', 'O1', 'O2'],
                        help='mixed precision opt level, if O0, no amp is used')
    parser.add_argument('--output', default='output/constraint', type=str, metavar='PATH',
...
def main(config):
    ...
    model = BYOL_Constraint_Wrapper(model, config)
    ...
    model_without_ddp = model.module.net
    ...
    scaling_factor_scheduler = ScalingHelper(
        scale_method=config.TRAIN.SCALING_SCHEDULER.METHOD,
        rate_initial=config.TRAIN.SCALING_SCHEDULER.MAX_RATE,
        min_rate=config.TRAIN.SCALING_SCHEDULER.MIN_RATE,
        step=config.TRAIN.START_EPOCH * len(train_loader) - 1,
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        train_epoch(config, model, criterion, train_loader, optimizer, epoch, mixup_fn, lr_scheduler, scaling_factor_scheduler)
...
def train_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, 
                lr_scheduler, scaling_factor_scheduler):
    model.train()
    ...
    for idx, ((view1, view2), targets) in enumerate(data_loader):
        view1 = view1.cuda(non_blocking=True)
        view2 = view2.cuda(non_blocking=True)
        targets = targets.cuda(non_blocking=True)

        if mixup_fn is not None:
            view1, targets1 = mixup_fn(view1, targets)
            view2, targets2 = mixup_fn(view2, targets)
        else:
            targets1, targets2 = targets, targets
        
        logits1, logits2, constraint = model((view1, view2))
        
        if config.TRAIN.ACCUMULATION_STEPS > 1:
            pred_loss = (criterion(logits1, targets1) + criterion(logits2, targets2)) / 2
            loss = scaling_factor_scheduler.scale(pred_loss, constraint)
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            logger.info(
                ...
                f'factor {scaling_factor_scheduler.factor:.4f}\t'
