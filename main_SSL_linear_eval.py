...
from utils import load_checkpoint_no_resume, get_grad_norm, auto_resume_helper, reduce_tensor
...
def parse_args():
    parser = argparse.ArgumentParser('Script for Linear Evaluating Self-supervised learning models', add_help=False)
    parser.add_argument('--general-cfg', type=str, default='configs/SSL_linear_eval.yaml', help='path to general config file')
    ...
    parser.add_argument('--linear-eval', default=True, help='Perform linear evaluation on SSL')
...
if __name__ == '__main__':
    ...
    logger = create_logger(output_dir=config.OUTPUT, dist_rank=dist.get_rank(), name=f"{config.MODEL.ARCH}_EVAL")
    
    if dist.get_rank() == 0:
        path = os.path.join(config.OUTPUT, "config_eval.json")
